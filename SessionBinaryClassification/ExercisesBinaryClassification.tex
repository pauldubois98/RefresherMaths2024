...
\begin{document}
	...
	% distance: $\frac{\mid \mathbf{w} \cdot \mathbf{x} - b \mid}{\mid \mid \mathbf{w} \mid \mid}$	
	...
	% need: $y_k \cdot h(\mathbf{x}_k) > 0 \ \forall k$.
	...
	% margin size: $\frac{2}{\mid \mid \mathbf{w} \mid \mid}$
	
	\noindent \textbf{\textit{TODO:}}
	Formulate the constrained optimization problem to find the best (i.e. maximizing the margin) parameters $\mathbf{w}$ and $b$.
	% problem: $\text{minimize}_{\mathbf{w}, b} \mid \mid \mathbf{w} \mid \mid$
	% such that $y_k (\mathbf{w}.\mathbf{x}_k - b) \geq -1 \quad \forall k$
